...
def detect_video(current_user):
    ...
    if not allowed_file(file.filename, allowed_extensions):
        return bad_request(f'不支持的视频格式，支持: {", ".join(allowed_extensions)}')
    ...
    # 执行检测
    detect_result = yolo_service.detect_video(save_path, result_dir, save_result=save_result)

    # 构建结果路径 URL
    result_video_url = None
    if detect_result.get('result_path'):
        static_dir = os.path.join(current_app.root_path, 'static')
        result_video_url = get_relative_url(detect_result['result_path'], static_dir)

    original_video_url = get_relative_url(save_path, os.path.join(current_app.root_path, 'static'))
    ...
    history = DetectionHistory(
        ...
        type='video',
        original_filename=original_name,
        original_path=original_video_url,
        result_path=result_video_url,
        detection_count=detect_result.get('detection_summary', {}).get('frames_with_objects', 0),
        file_size=file_size,
        processing_time=detect_result['processing_time'],
        model_version=detect_result['model_version']
    )
    history.set_detection_result(detect_result.get('detection_summary', {}))
    db.session.add(history)
    db.session.commit()
    ...
    return success(data={
        ...
        'frame_count': detect_result.get('frame_count'),
        'detection_summary': detect_result.get('detection_summary'),
        'processing_time': detect_result['processing_time'],
        'model_version': detect_result['model_version']
    }, message='检测完成')
