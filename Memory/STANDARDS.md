...
用户  →  给我任务需求
   ↓
我（管理端）  →  写任务文档到 Projects/项目名/docs/*任务-MMDD.md
   ↓
用户  →  把简短指令发给开发 Task
         - "执行后端任务 MMDD"
         - "执行前端任务 MMDD"
   ↓
开发 Task  →  读取文档 → 执行开发 → 完成代码
   ↓
开发 Task 完成后  →  我回来验证 → 生成验证报告
   ↓
代码合并  →  Git 提交日志
```
...
| 角色 | 职责 |
|------|------|
| 我（管理端） | 需求分析、编写任务文档、验证功能、更新规范 |
| 用户 | 分发指令给开发 Task |
| 开发 Task | 读取文档、执行开发、自测验证 |
